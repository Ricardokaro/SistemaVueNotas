  ...
  <v-layout align-start>
    <v-flex>
      <v-toolbar flat color="white">
        <v-toolbar-title>Año Escolar</v-toolbar-title>
        ...
        <v-dialog v-model="dialog" max-width="500px">
          <template v-slot:activator="{ on }">
            <v-btn color="primary" dark class="mb-2" v-on="on">Nuevo Año Escolar</v-btn>
          </template>
          <v-card>
            <v-card-title>
            ...
            <v-card-text>
              <v-container grid-list-md>
                <v-layout wrap>
                  ...
                  <v-flex xs6 sm6 md6>
                    <v-text-field v-model="anio" label="Año escolar" disabled></v-text-field>
                  </v-flex>
      ...
      <v-data-table :headers="headers" :items="anioescolares" :search="search" class="elevation-1">
...
<script>
  ...
  export default {             
      data(){
          return{
              ...
              headers: [
                  ...
                  { text: 'Anio', value: 'anio', sortable: false },
      ...
      computed: {
          formTitle () {
              return this.editedIndex === -1 ? 'Nuevo Año Escolar' : 'Editar Año Escolar'
          },
          computedTomarAnio () {
             this.anio = this.formatTomarAnio(this.fecha_inicio)
          },
          computedFechaMinima(){
      ...
      methods:{          
          listar(){  
            ...
            axios.get('api/Anioescolares/Listar', configuration).then(function(response){               
               me.anioescolares = response.data;
          ...
          activar(){
             ...
             let header = {"Authorization" : "Bearer " + this.$store.state.token};
             let configuration = {headers : header};  
                axios.put('api/Anioescolares/Activar/' + this.adId,{},configuration).then(function(response){
                    me.adModal = 0;
                    me.adAccion = 0;
                    me.adNombre = "";
                    me.adId = "";
          ...
          desactivar(){
              ...
              let header = {"Authorization" : "Bearer " + this.$store.state.token};
              let configuration = {headers : header}; 
                axios.put('api/Anioescolares/Desactivar/' + this.adId,{},configuration).then(function(response){
                    me.adModal = 0;
                    me.adAccion = 0;
                    me.adNombre = "";
                    me.adId = "";
          ...
          guardar () {
              ...
              if (this.editedIndex > -1) {
                  //codigo para editar                  
                   let me = this;
                   axios.put('api/Anioescolares/Actualizar',{
                   'idanio_escolar': me.id,
                   'nombre': me.nombre,
                 ...
                 //codigo para guardar
                 let me = this;
                 axios.post('api/Anioescolares/Crear',{
                   'nombre': me.nombre,
                   'fecha_inicio': me.fecha_inicio,
                   'fecha_final': me.fecha_final,
                   'anio': me.anio        
          ...
          validar(){
              ...
              if(this.nombre.length < 3 || this.nombre.length > 50){
                  this.validaMensaje.push("El nombre debe tener mas de 3 caracteres y menos de 50 caracateres")
              }
